...
pos = array([0,0,0])
pos.shape = (1,3)
pos = y + mu_est_hsmm*delt

for j in A:
    temp = pos[j]
    temp.shape = (1,3) 
    mu_est_hsmm, sigma_est_hsmm = hsmm.condition(temp, dim_in=slice(0, 3), dim_out=slice(3, 6))
    newpos = temp + mu_est_hsmm*delt
    newpos.shape = (1,3)
